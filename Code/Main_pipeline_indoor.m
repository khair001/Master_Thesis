%% Radar Detection, Classification, Clustering, and Bearing Estimation
% Processes raw ADC recordings from the TI IWR6843ISK/DCA1000 platform.
% The pipeline performs frame-level interference detection, dual-path
% CA-CFAR detection, rule-based classification, DBSCAN characterisation,
% range-azimuth visualisation, and exploratory interference bearing
% estimation from RX1/RX4 phase differences.
%
% Update testFile, numADCSamples, freqSlope, and true_bearing_deg for the
% dataset under evaluation. The 128- and 256-sample configurations use
% separate empirically derived thresholds.

clear; clc; close all;


%% Dataset configuration
% Raw ADC file captured with the observer radar.
testFile         = 'adc_data_Raw_staticmoving.bin';
% ADC samples per chirp; supported values are 128 and 256.
numADCSamples    = 256;   % set to 128 or 256 to match the dataset

% Ground-truth interferer bearing used only for evaluation.
...
% Bilateral frame-power spike sensitivity.
...
% Waveform parameters; these must match the captured radar profile.
fc          = 60e9;           % carrier frequency (Hz)
freqSlope   = 69.994e6 / 1e-6;% chirp frequency slope (Hz/s), 29.982/30.0179/69.994
...
% Assumed horizontal RX1/RX4 element spacing.
d_ant       = lambda / 2;
...
%% Configuration-specific thresholds
...
% Minimum range supported by the CFAR window geometry.
first_valid_range = (first_valid_bin - 1) * rangeRes;

% Thresholds are configured separately for the two ADC sample settings.
if numADCSamples == 128
    ...
    snr_static_min       = 12;                % minimum SNR to be labelled Static (dB)
    snr_moving_min       = 12;                % minimum SNR to be labelled Moving (dB)
    snr_interference_min = 9;                 % minimum SNR to be promoted to Interference (dB)
    ...
    min_spike_count      = 2;                 % minimum frame-power spikes to declare interference
...
% Bearing-quality thresholds.
bearing_snr_min      = 25;   % dB — minimum SNR for a detection to contribute

% Circular-standard-deviation limits for qualitative confidence labels.
...
% Hann windows applied along range and Doppler dimensions.
...
% Bytes per complete ADC frame.
bytesPerFrame = numADCSamples * numRX * numChirps * 4;

% Zero-velocity bin after fftshift.
zeroVelBin    = ceil(numChirps/2) + 1;

%% Step 1: Frame-power calculation and interference spike detection
% A frame is flagged when its mean Range-Doppler power exceeds both
% neighbouring frames by the configured sensitivity margin.
...
for f = 1:numFramesToAudit
    ...
    % [I_lane1, I_lane2, Q_lane1, Q_lane2], with RX channels interleaved.
...
% Detect bilateral local frame-power spikes.
...
% Apply the minimum-spike-count declaration criterion.
...
% RX1 provides the detection map. RX1/RX4 complex values are retained for
% phase-based azimuth and later interference-bearing estimation.
fprintf('--- STEP 2: Processing %d frames ---\n', Total_Audited);
fid = fopen(testFile, 'r');
if fid == -1, error('File not found: %s', testFile); end

% Detection rows: [frame, range, velocity, SNR, peak power, azimuth].
frame_dets = cell(Total_Audited, 1);

% Preserve RX1/RX4 complex values for coherent bearing estimation.
...
% Accumulate Range-Doppler maps for the time-averaged display.
...
for f = 1:Total_Audited
    ...
    % Extract one complex fast-time/slow-time matrix per RX channel.
    rx = cell(4,1);
    for rxIdx = 1:numRX
    ...
    % RX1 magnitude map used for CFAR and SNR calculation.
    rd_db = 20*log10(abs(rd_complex_rx1) + eps);

    % Update the frame-averaged display map.
    rd_map_accum       = rd_map_accum + rd_db;
    frames_accumulated = frames_accumulated + 1;

    % Estimate the frame noise reference from the RX1 map.
    ...
    % Preallocate the frame detection mask.
    ...
    if ~isempty(rBins)
        ...
        % --- Phase-based azimuth estimation (RX1 referenced against RX4) ---
        ...
        phase_diff = angle(rx1_val .* conj(rx4_val));       % wrapped to [-π, π] -- RX1 referenced against RX4
...
% Time-averaged Range-Doppler map.
rd_map_avg = rd_map_accum / frames_accumulated;

% Concatenate frame detections for downstream processing.
...
% Concatenate complex values in the same order as allDets.
rx1_complex_all = cell2mat(rx1_complex_store(1:Total_Audited));
rx4_complex_all = cell2mat(rx4_complex_store(1:Total_Audited));

% Cartesian companion coordinates derived from the same range/azimuth
% estimates for floor-plan visualisation only.
...
% Detections are labelled Static, Moving, or Noise using the physical range
% gate, SNR thresholds, and absolute radial velocity. Noise detections remain
% eligible for interference promotion in Step 4.
...
for k = 1:detCount
    ...
    if snr_k < snr_noise_thresh
        % Below the global SNR gate.
        labels(k) = "Noise";
    elseif vel_k < vel_static_thresh && snr_k >= snr_static_min
        % Low-velocity, high-SNR static candidate.
        labels(k) = "Static";
    elseif vel_k >= vel_moving_min && vel_k <= vel_moving_max ...
           && snr_k >= snr_moving_min
        % Pedestrian-band moving candidate.
        labels(k) = "Moving";
    else
        % Remaining detections enter the Noise residual.
        labels(k) = "Noise";
    end
end
...
% Noise detections are promoted to Interference only when they are
% temporally correlated with a detected spike, have anomalous velocity,
% and exceed the interference SNR threshold.
if Interfered_Count >= min_spike_count
    ...
    % Include the immediate neighbours of each detected spike frame.
    ...
    % Candidate set for Noise-to-Interference promotion.
    ...
    for k = noiseIdx'
        ...
        % Gate 1: temporal correlation with a spike event.
        ...
        % Gate 3: interference SNR threshold.
        if snr_k < snr_interference_min, continue; end

        % Promote detections satisfying all three gates.
        labels(k) = "Interference"; promoted = promoted+1;

        % Record diagnostic signature statistics.
...
% Clustering is performed in normalised [range-bin, Doppler-bin] space.
% Interference members assigned to a DBSCAN cluster are labelled clustered
% interference; DBSCAN outliers remain isolated interference.
...
% Compute mean range, velocity, SNR, azimuth, and member count for each
% confirmed Static, Moving, and clustered-interference group.
fprintf('--- Cluster Centroids ---\n');
fprintf('%-22s %6s %10s %10s %8s %8s %8s\n', ...
    'Class','ClustID','Range(m)','Vel(m/s)','SNR(dB)','Az(deg)','Count');
fprintf('%s\n', repmat('-',1,72));

% centroid_list = [classIndex, clusterID, range, velocity, SNR, azimuth, count].
centroid_list = [];

% Plot metadata indexed by class.
colMap = {col_static, col_moving, col_interf};
lblMap = {'Static centroid','Moving centroid','Clustered Interference centroid'};

% Class-specific cluster-ID offsets.
...
% The RX1/RX4 coherent phase product is averaged in the complex domain to
% avoid direct averaging of wrapped phase angles. With the assumed
% half-wavelength spacing, theta = asin(delta_phi/pi).
% Convention: 0 deg = ahead, negative = left, positive = right.
...
if nConfirmedInterf > 0
    ...
    for ci = 1:nClusters
        ...
        % --- Coherent phase averaging (RX1 referenced against RX4) ---
        ...
        phase_products = rx1_cluster .* conj(rx4_cluster);   % RX1 referenced against RX4
        coherent_sum   = sum(phase_products);

        % Convert coherent phase difference to bearing.
        ...
        % Store per-cluster bearing results.
    ...
    %% Step 6c: Scene-level bearing aggregation
    % Report both unweighted and SNR-weighted circular mean estimates.
    ...
    % Confidence label based on inter-cluster spread.
    ...
    %% Step 6d: Frame-level bearing distribution
    % Compute an independent coherent bearing estimate for each spike frame.
    ...
    for fi = 1:numel(spike_frames)
        ...
        % (RX1 referenced against RX4, matching Step 2 / Step 6b)
        rx1_f     = rx1_complex_all(frame_interf_idx);
        rx4_f     = rx4_complex_all(frame_interf_idx);
        coh_sum_f = sum(rx1_f .* conj(rx4_f));   % RX1 referenced against RX4
    ...
    if numel(valid_frame_bearings) > 1
        % Circular statistics across valid frame-level estimates.
    ...
    % Bearing estimation requires at least one confirmed interference cluster.
    ...
    % Sentinel values for downstream reporting and plots.
...
% In-range detection mask.
inRange = det_range >= minPhysicalRange & det_range <= maxPhysicalRange;

% Detection counts used by visualisations.
...
% Figure 1: Frame power and detected interference spikes.
...
% Figure 2: Range-versus-velocity classification view.
...
% Figure 3: Range-azimuth polar detection map.
...
% Figure 3b: Cartesian floor-plan view derived from range and azimuth.
...
% Figure 4: Three-dimensional azimuth-range-SNR view.
...
% Figure 5: Per-frame classified-detection counts.
